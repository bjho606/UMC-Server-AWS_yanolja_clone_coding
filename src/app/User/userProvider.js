...
exports.retrieveUserList = async function (nickname) {
  ...
  //nickname을 인자로 받는 경우와 받지 않는 경우를 구분하여 하나의 함수에서 두 가지 기능을 처리함
...
exports.retrieveUserPointsList = async function (id) {
  const connection = await pool.getConnection(async (conn) => conn);
  const userPointsUsage = await userDao.getUserPointsList(connection, id);
  connection.release();

  return userPointsUsage;
}